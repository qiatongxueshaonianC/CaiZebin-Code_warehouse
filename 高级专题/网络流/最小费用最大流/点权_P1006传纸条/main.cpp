...
#include<iomanip>
using namespace std; 
const int maxn=2*50*50+100;		//因为要拆点 
...
int main() {
	...
	s=hashin(1,1)-1;
	t=hashout(N,M)+1;
	InsertEdge(s,hashin(1,1),2,0);	//起点连到左上角的点 
	InsertEdge(hashout(N,M),t,2,0);	//右下角的点连到终点 
	for(int i=1;i<=N;i++){
		for(int j=1;j<=M;j++){
			int from=hashin(i,j);
			int to=hashout(i,j);
			InsertEdge(from,to,1,-g[i][j]);		//拆成两个点，并且连起来，这个是针对点权的 
			InsertEdge(from,to,1,0);		//有可能第二次通过 
			if(i!=N)
