...
void solve(){
	for(auto e:edges){
		...
		if(fa>fb)
			swap(fa,fb);  //方便最后输出时要小的在前面 
		if(fa!=fb&&e.cost<=node[fa].h&&e.cost<=node[fb].h){ //2020/2/9/01：17发现，两个组件是 
			node[fa].father+=node[fb].father;		//大于，所以合并是小于等于 
			node[fb].father=fa;
